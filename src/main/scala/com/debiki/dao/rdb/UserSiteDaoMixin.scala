...
trait UserSiteDaoMixin extends SiteDbDao with SiteTransaction {
  ...
  def updateCompleteUser(user: CompleteUser): Boolean = {
    ...
    val values = List(
      ...
      user.tinyAvatar.map(_.hashPathSuffix).orNullVarchar,
      user.smallAvatar.map(_.baseUrl).orNullVarchar,
      user.smallAvatar.map(_.hashPathSuffix).orNullVarchar,
      user.mediumAvatar.map(_.baseUrl).orNullVarchar,
      user.mediumAvatar.map(_.hashPathSuffix).orNullVarchar,
